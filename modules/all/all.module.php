...
class All{
	...
	public function index($page = 1, $user_id = 0, $flag_no_view = 0, $flag_no_moder = false, $flag_vip_add = 0, $error = false, $flag_no_show = 0) {
        ...
        //$news = ModelAll::getNews($page, $limit, $user_id, null, $flag_no_view, ($flag_no_moder ? 0 : 1), $flag_vip_add, ($flag_no_show ? 0 : 1));
		$news = ModelAll::getNews($page, $limit, $user_id, null, $flag_no_view, ($flag_no_moder ? 0 : 1), $flag_vip_add, ($flag_no_show ? 0 : 1));
		//Site::d($news);
	
		  
		if ($flag_vip_add) {
			$vipRequests = ModelAll::getVipRequests();
		...
		echo Registry::get('twig')->render($tpl_name, array(
			'articles'		=> ModelAll::getArticlesList(),
			'news'			=> $news,
			'count_f'       => count($news),
			'pagination'	=> $pagination,
            'error'         => $error,
            'topProviders'  => ModelAll::getTopProviders() 
		));
	}
	...
	public function search($q, $page = 1) {
        ...
        $result = ModelAll::getNews($page, 10, 0, $q);

		echo Registry::get('twig')->render('search.tpl', array(
			'q'					=> $q,
			'categs'			=> ModelAll::searchInCategs($q),
	...
	public function journals() {
		...
		echo Registry::get('twig')->render('journals.tpl', array(
			'journals'	=> ModelAll::getJournalsList()
		));
	}
	...
	public function getJournalPages($journal_id) {
		...
		$result = ModelAll::getJournalPages($journal_id);
	...
	public function getMessTpl($mess_id) {
		...
		echo json_encode(array(
			'message' => ModelAll::getMessTpl($mess_id)
		));
	}
	...
	public function vipRequestConfirm($section_id, $resource_id, $type = 1) {
		if ($section_id > 0 and $resource_id > 0) {
			...
			ModelAll::updateFlagVipAdd($table, $primary_key, $resource_id);
			ModelAll::updateVipRequest($section_id, $resource_id, $type);
	...
	public function vipRequestDelete($section_id, $resource_id) {
		...
		ModelAll::updateFlagVipAdd($table, $primary_key, $resource_id, 0);
		
		ModelAll::updateVipRequest($section_id, $resource_id, 0, 1);
	...
	public function rss($country_id = 1, $page = 1, $count = 10) {
		...
		$news = ModelAll::getNewsList($country_id, $page, $count, 0, null);
    ...
    public function providerTransition($providerId, $link) {
        ...
        ModelAll::setProviderTransition($providerId);
