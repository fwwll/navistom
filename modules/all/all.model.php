    ...
    public function getNews($page = 1, $count = 10, $userId = 0, $q = null, $flagModerView = 0, $flagModer = 1, $flagVipAdd = 0, $flagShow = 1)
    {
        ...
        FROM (
          (SELECT
               ...
               activity.user_name,
               activity.name,
			   IF(l.image, CONCAT('lectors/', l.image), CONCAT('142x195/', im.url_full)) AS image,
           ...
           SELECT
               ...
               IFNULL(CONCAT('offers/142x195/', i.url_full), CONCAT('products/80x100/', products.image)) AS image,
       ...
       ".((User::isAdmin()or true ) ?'LEFT JOIN' :'INNER JOIN') ." top_to_main AS c ON c.section_id = a.section_id AND c.resource_id = a.content_id AND c.date_start <= '$now' AND c.date_end > '$now'
        ". ((User::isAdmin()) ? "
