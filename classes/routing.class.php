...
class routing
{
	...
	*/
	public function __construct ($file, $cache_dir, $debug = false)
	{
		$cache_file = $cache_dir . DIRECTORY_SEPARATOR . 'routing-cache-' . md5($file) . '.dat';
		
		if (!$debug && file_exists($cache_file) ) {
			$this->route = unserialize(file_get_contents($cache_file));
		...
		foreach ($xml->children() as $route) {
			...
			if(strpos($params['match'], '{') !== false) {
				...
				foreach($match_attrs as $key => $varible) {
					...
					$params['match'] = str_replace("{" . $varible[1] . "|str}", '([а-яёa-z1-9_-]*)', $params['match']);
