...
<div class="idTabs">
    ...
    <div class="print-btn">
        <noindex>
            ...
            <div target="_blank" class='color727272 print'><i class="print-new-iconz"></i> На печать</div>
        </noindex>
    </div>
    <p class='clear'>&nbsp;</p>
    {% if interview %}
        ...
        <div id="comment-list">
            {% for c in comments %}
                <div class="n-comment a-clear">
                    <div class="col-1">
                        <img title="{{ c.name }}" alt="{{ c.name }}" src="/uploads/users/avatars/tumb2/{{ c.avatar }}"/>
                    </div>
                    <div class="col-2">
                        <div class="a-row">
                            <div class="a-cols-2">
                    ...
                    {% else %}
                        {% if isBigImg(image,1) %}
                            <img alt="{{ offer.name }}" title="{{ offer.name }}"
                                 src="{{ image|default('/uploads/images/100x80.jpg') }}">
                        {% else %}
                            {% set image= image|replace({'142x195':'full'}) %}
                            {% if isBigImg(image,1) %}
                                <img alt="{{ offer.name }}" title="{{ offer.name }}" src="{{ image }}">
                            {% else %}
                                <img alt="{{ offer.name }}" title="{{ offer.name }}"
                                     src="{{ image|replace({'full':'80x100'}) }}">
                            {% endif %}
                        {% endif %}
                    {% endif %}
                    <div class="content">
