...
{% block content %}
    ...
    {% else %}
        ...
        <li>
            ...
            {% else %}
                <span class='label_center'> Золотой фон на все время размещения + Разместить вверху списка</span>
        ...
        <li>
            ...
            {% else %}
                <span class='label_center'>Метка на все время размещения + Разместить вверху списка</span>
                <img src='/templates/Navistom/images/vip/galochka-check_24x24.png'/>
            {% endif %}
        </li>
        <li>
    ...
    <li style='height:243px;'>
        ...
        <span class='icon_img '><img src='/templates/Navistom/images/vip/magazine-new-icon.png'/> </span>
			  <span class='l_m_f'><a target='_blank' href='/journals' class='' style='display:inline-block'>
                      Опубликовать объявление в нашем Журнале </a>
			  </span>
        ...
        <div class='foto_success'>
            <div class='textf'>
			   <span> Примерный вид этого обявления в журнале:<br/>
					(фото + заголовок + цена + телефон)</span>

                <div class='bag_img_su'>
                    <div class='img_j'>
                        {% if img_j.image %}
                            <img src='{{ img_j.path }}{{ img_j.image }}'/>
                        {% else %}
                            <img src='/uploads/images/100x80.jpg'/>
                        {% endif %}
                    </div>
                    <span class='name_j'  {% if img_j.section_id=='5' %} style='font-size:14px;' {% endif %}>{{ img_j.name }}</span>
                    {% if img_j.price %}
                        <span class='price_j'>{{ img_j.price|getExchangeRates(img_j.currency_id, img_j.user_id)|number_format(2, '.', ' ') }} {{ default_currency }}</span>
                        <br/>
                    {% endif %}
                    <span class='tel_j'>{{ img_j.phones }}</span>
            ...
            <div class='action_j'>
                <p class='p_j'>
                    <br><br>Все ТОП-объявления публикуем
                    <br>в Журнале и Соцсетях бесплатно!
                </p>
            </div>
