...
{% block content %}
    ...
    {% if data.user_id == user_info.info.user_id or is_admin %}
        ...
        <form id="activity-add-form" class="n-add-form a-clear" method="post" enctype="multipart/form-data"
              action="/index.ajax.php?route=/work/resume/edit_ajax-{{ data.work_id }}">
        <div class="a-row">
        ...
        <div class="a-row">
            ...
            <input value="{{ data.user_surname }}" class="validate[required]" type="text" name="user_surname"
                   id="user_surname"/>
        </div>
        <div class="a-row">
        ...
        <div class="a-row">
            ...
            <input type="checkbox" {% if data.leave_type > 0 %}checked="checked"{% endif %}  name="leave" id="leave"
                   value="1"/>
        </div>
        <div class="a-row">
        ...
        <div class="a-row">
            ...
            <select class="validate[required]" id="employment_type" name="employment_type">
                <option {% if data.employment_type == 1 %}selected="selected"{% endif %} value="1">полная занятость
                </option>
                <option {% if data.employment_type == 2 %}selected="selected"{% endif %} value="2">неполная занятость
                </option>
                <option {% if data.employment_type == 3 %}selected="selected"{% endif %} value="3">удаленная работа
                </option>
                <option {% if data.employment_type == 4 %}selected="selected"{% endif %} value="4">посменно</option>
            </select>
        </div>
        ...
        <div id="resume-work">
            <div class="a-row a-form-mess {% if employments|length > 0 %}display-none{% endif %}">
                ...
                Заполняя информацию об опыте работы, постарайтесь раскрыть для работодателя ваши положительные стороны,
                которые в наибольшей мере соответствуют заголовку вашего резюме. Опишите ваши обязанности, но не
                останавливайтесь только на них. Не забудьте упомянуть о ваших знаниях, умениях и о положительном опыте
            ...
            {% for e in employments %}
                <div class="resume-add-work-parent" id="resume-add-work-{{ loop.index - 1 }}">
                    ...
                    <div class="a-row">
                        ...
                        <input value="{{ e.company_name }}" class="validate[required]" type="text"
                               name="work[company_name][]" id="company_name_{{ loop.index - 1 }}"/>
                    </div>
                    <div class="a-row">
                        <label><font class="a-red">*</font> Должность</label>
                        <input value="{{ e.position }}" class="validate[required]" type="text" name="work[position][]"
                               id="position_{{ loop.index - 1 }}"/>
                    </div>
                    <div class="a-row">
                        <label><font class="a-red">*</font> Выполняемые обязаности</label>
                        <input value="{{ e.activity }}" class="validate[required]" type="text" name="work[activity][]"
                               id="activity_{{ loop.index - 1 }}"/>
                    </div>
                    <div class="a-row">
                        <label><font class="a-red">*</font> Период работы</label>
                        <input value="{{ e.date_start }}" placeholder="Дата начала" type="text"
                               class="month-picker month-picker-start validate[required]" name="work[date_start][]"
                               id="work_date_{{ loop.index - 1 }}"/>
                        <i class="a-icon-calendar"></i>
                        <input value="{{ e.date_end }}" placeholder="Дата окончания" type="text"
                               class="month-picker validate[required]" name="work[date_end][]"
                               id="work_date_{{ loop.index - 1 }}"/>
                        <i class="a-icon-calendar"></i>
                    </div>
                    <hr class="hr-min"/>
        ...
        <div id="resume-experience">
            <div class="a-row a-form-mess {% if educations|length > 0 %}display-none{% endif %}">
                <i class="a-icon-info-sign"></i>
                Во время редактирования этого блока, вы можете добавить в резюме образование, которое вы получили или о
                которой хотите упомянуть в соответствии с целями данного резюме.
                <p>
            ...
            {% for e in educations %}
                <div style="position:relative;" class="resume-add-experience-parent"
                     id="resume-experience-{{ loop.index - 1 }}">
                    <a href="javascript:void(0)" class="delete-resume-added">Удалить</a>
                    ...
                    <div class="a-row">
                        ...
                        <select class="validate[required]" name="education[type][]" id="type_rid_{{ loop.index - 1 }}">
                            ...
                            <option {% if e.type == 2 %}selected="selected"{% endif %} value="2">неоконченное высшее
                            </option>
                            <option {% if e.type == 3 %}selected="selected"{% endif %} value="3">среднее специальное
                            </option>
                    ...
                    <div class="a-row">
                        ...
                        <input value="{{ e.institution }}" class="validate[required]" type="text"
                               name="education[institution][]" id="institution_{{ loop.index - 1 }}"/>
                    </div>
                    <div class="a-row">
                        <label><font class="a-red">*</font> Факультет, специальность</label>
                        <input value="{{ e.faculty }}" class="validate[required]" type="text"
                               name="education[faculty][]" id="faculty_{{ loop.index - 1 }}"/>
                    </div>
                    <div class="a-row">
                        <label><font class="a-red">*</font> Город</label>
                        <input value="{{ e.location }}" class="validate[required]" type="text"
                               name="education[location][]" id="location_{{ loop.index - 1 }}"/>
                    </div>
                    <div class="a-row">
                        <label><font class="a-red">*</font> Период обучения</label>
                        <input value="{{ e.date_start }}" placeholder="Дата начала" type="text"
                               class="month-picker month-picker-start validate[required]" name="education[date_start][]"
                               id="education_date_start_{{ loop.index - 1 }}"/>
                        <i class="a-icon-calendar"></i>
                        <input value="{{ e.date_end }}" placeholder="Дата окончания" type="text"
                               class="month-picker validate[required]" name="education[date_end][]"
                               id="education_date_end_{{ loop.index - 1 }}"/>
                        <i class="a-icon-calendar"></i>
                    </div>
                    <hr class="hr-min"/>
                </div>
        ...
        <div id="resume-traning">
            ...
            {% for t in tranings %}
                <div style="position:relative;" class="resume-add-traning-parent"
                     id="resuma-traning-{{ loop.index - 1 }}">
                    <a href="javascript:void(0)" class="delete-resume-added">Удалить</a>

                    <div class="a-row">
                        <label><font class="a-red">*</font> Название учебного заведения (курсов)</label>
                        <input value="{{ t.name }}" class="validate[required]" type="text" name="traning[name][]"
                               id="traning_name_{{ loop.index - 1 }}"/>
                    </div>
                    <div class="a-row">
                        <label><font class="a-red">*</font> Год, длительность</label>
                        <input value="{{ t.description }}" class="validate[required]" type="text"
                               name="traning[description][]" id="traning_year_{{ loop.index - 1 }}"/>
                    </div>
                    <hr class="hr-min"/>
                </div>
            {% endfor %}
        ...
        <div id="resume-langs">
            <div class="a-row a-form-mess {% if langs|length > 0 %}display-none{% endif %}">
                <i class="a-icon-info-sign"></i>
                Используйте этот блок резюме, чтобы указать, какие языки вы знаете, и оценить степень владения ими. Даже
                если знание языков напрямую и не требуется в вакансиях, на которые вы претендуете, эта информация будет
                полезна как дополнительная.
                <p>
            ...
            {% for l in langs %}
                <div style="position:relative;" class="resume-add-lang-parent" id="resume-lang-{{ loop.index - 1 }}">
                    ...
                    <div class="a-row">
                        ...
                        <input value="{{ l.name }}" class="validate[required]" type="text" name="langs[name][]"
                               id="lang_name_{{ loop.index - 1 }}"/>
                    </div>
                    <div class="a-row">
